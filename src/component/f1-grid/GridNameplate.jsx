...
export default function GridNameplate({ isInGrid, doSwap, driverInfo, swapCar, teamWith, isSwappingCar, isSwappingPoints, teamWithVictimId }) {
                        ...
                        <>
                            {teamWith > 0 && !isSwappingCar &&
                                <div className="nameplate-swap-points-container" onClick={() => doSwap('teamWith', driverInfo.playerId)}>
                                    ...
                                    {isSwappingPoints
                                        ? <span style={{ fontSize: '12px' }} className='spanF1'> Swapping... </span>
                                        : <span style={{ fontSize: '12px' }} className='spanF1'> Swap Points ({teamWith || 0}) </span>
                                    }
                                </div>
                            }
                            {teamWith > 0 && isSwappingCar &&
                            ...
                            {teamWith == 0 &&
                                <div className="nameplate-swap-points-container" style={{ backgroundColor: 'gray' }}>
                                    <img src={swapcar} height="18px" width="18px" alt="swap points" /><span style={{ fontSize: '12px' }}> No Team Withs</span>
                                </div>
                            }
                            ...
                            {swapCar > 0 && !isSwappingPoints &&
                                <div className="nameplate-swap-car-container" onClick={() => doSwap('swapCar', driverInfo.playerId)}>
                                    ...
                                    {isSwappingCar
                                        ? <span style={{ fontSize: '12px' }} className='spanF1'> Swapping...</span>
                                        : <span style={{ fontSize: '12px' }} className='spanF1'> Swap Car ({swapCar})</span>}
                                </div>
                            }
                            {swapCar > 0 && isSwappingPoints &&
